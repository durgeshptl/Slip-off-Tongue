const API_URL = "https://slip-off-tongue.onrender.com/predict";
...
async function analyze() {
  ...
  try {
    ...
    alert("Oops! Could not connect.\n\nPossible reasons:\n• Render is sleeping (wait 10–30 sec and try again)\n• Internet issue\n• Server error\n\nCheck console (right-click popup → Inspect → Console) for details.");
...
// Attach button events + auto-fill + auto-analyze when popup loads
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Auto-fill selected text and auto-analyze if text exists
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    chrome.scripting.executeScript({
      ...
      if (selected) {
        ...
        analyze();  // auto run analysis when text is selected
      }
    });
  });
});